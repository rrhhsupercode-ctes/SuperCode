  ...
  function verMovimientoModal(id) {
    (async () => {
      ...
      (mov.items || []).forEach(it => {
        html += `<hr id="hr-ticket"><p id="texto-ticket">${escapeHtml(it.nombre)} <br>Unidad: ${formatoPrecioParaPantalla(it.precio)}</p> <p id="texto-ticket">(x${it.cantidad}) = </p> <p id="texto-ticket"> ${formatoPrecioParaPantalla(it.precio * it.cantidad)}</p>`;
      });
