...
async function eliminarMovimiento(id) {
  const pass = prompt("Ingrese contraseña administrativa para eliminar:");
...
async function eliminarStock(codigo) {
  const pass = prompt("Ingrese contraseña administrativa para eliminar producto:");
  const snapConfig = await window.get(window.ref(window.db, "config/passAdmin"));
  const adminPass = snapConfig.exists() ? snapConfig.val() : ADMIN_PASS_DEFAULT;
...
async function eliminarCajero(nro) {
  const pass = prompt("Ingrese contraseña administrativa para eliminar cajero:");
  const snapConfig = await window.get(window.ref(window.db, "config/passAdmin"));
  const adminPass = snapConfig.exists() ? snapConfig.val() : ADMIN_PASS_DEFAULT;
